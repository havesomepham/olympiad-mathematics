...
\end{problem*}
\begin{proof}
  We claim this fixed point is the orthocenter of \(\triangle ABC\). Call this point $H$. Note that  \(\ol{PX} \parallel \ol{RB}\), \(\ol{QX} \parallel \ol{AH}\), and \(\ol{RX} \parallel \ell\). Because \(\ol{PX}, \ol{QX}, \text{ and } \ol{RX}\) all pass through the single point \(X\), the lines \(\ol{RB}, \ol{AH},\text{ and } \ell\) must also pass through a single point, which is easy to see by translation of \(X\). Since \(H\) lies on both \(\ol{AH}\) and \(\ol{RB}\) by construction of the orthocenter, \(\ell\) must also pass through the orthocenter as well. Note that this argument holds for any placement of $X$ along minor arc $\widehat{BC}$, so we are done.
\end{proof}
\end{document}
...
\begin{problem*}
  [4.30]
  Prove (g) of Lemma 4.26. 
\end{problem*}
\begin{proof}
  What we want to prove is $\ol{BC}$ is the interior angle bisector of $\angle AMK,$ and $\ol{MK}$ is the exterior angle bisector. It suffices to show that $\dang AMB = \dang BMK.$ By (b) we have that
  $$\dang AMB = \dang AMC = \dang ABK$$ 
\end{proof}
\end{document}
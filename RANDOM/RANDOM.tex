...
\end{problem*}
\begin{soln}
  (Not entirely original) Denote by \(D\) the foot of the altitude from \(B\) to \(AC\). With some simple calculations we find that \(AD = 5\), \(BD = 12\), and \(MD = 2\). Then by the Pythagorean Theorem we find that \(BM = 2\sqrt{37}\). Denote by \(E\) the foot of the altitude from \(P\) to \(AC\). Then we know that \(\triangle PME \sim \triangle BMD\) by \(AA\sim\). So,
